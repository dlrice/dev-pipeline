...
function detect(cwd) {
  ...
  // Check for existing aidev files (to warn before overwriting)
  const aidevFiles = [
    'aidev.config.json',
    'aidev/specs/_template.md',
  ];
  result.existingFiles = aidevFiles.filter(f => fs.existsSync(path.join(cwd, f)));
