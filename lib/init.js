...
async function init() {
  ...
  if (fs.existsSync(configPath)) {
    ...
    console.log('    Use --force to overwrite, or just run "aidev run <feature>".');
  ...
  const configObj = {
    ...
    complexity: {
      maxScreens: config.projectSize === 'small' ? 2 : config.projectSize === 'large' ? 8 : 4,
      maxNewComponents: config.projectSize === 'small' ? 3 : config.projectSize === 'large' ? 10 : 5,
      maxNewApiEndpoints: config.projectSize === 'small' ? 1 : config.projectSize === 'large' ? 6 : 3,
      maxModifiedFiles: config.projectSize === 'small' ? 5 : config.projectSize === 'large' ? 20 : 10,
      maxNewCodeLines: config.projectSize === 'small' ? 250 : config.projectSize === 'large' ? 1000 : 500,
      maxTests: config.projectSize === 'small' ? 20 : config.projectSize === 'large' ? 100 : 50,
    },
  ...
  if (!fs.existsSync(docsDir)) {
    ...
    const docStubs = ['ARCHITECTURE.md', 'CONVENTIONS.md', 'DATA-MODEL.md', 'DEPENDENCIES.md', 'DECISIONS.md'];
