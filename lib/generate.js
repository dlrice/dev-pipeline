...
function generate(cwd, config, { force = false } = {}) {
  ...
  const dirs = [
    'docs', 'src',
    'pipeline/specs', 'pipeline/plans', 'pipeline/scripts', 'pipeline/logs',
    'pipeline/signals',
    'pipeline/reviews/spec-critiques', 'pipeline/reviews/plan-reviews',
    'pipeline/reviews/adversarial',
    'pipeline/tests/gemini', 'pipeline/tests/qwen', 'pipeline/tests/adversarial',
  ...
  writeIfNew(cwd, 'pipeline/specs/_template.md', readTemplate('specs/_template.md'), force, written, skipped);
  ...
  writeAlways(cwd, 'pipeline/scripts/dev-loop.sh', readTemplate('scripts/dev-loop.sh'), written);
  writeAlways(cwd, 'pipeline/scripts/setup.sh', readTemplate('scripts/setup.sh'), written);
  writeAlways(cwd, 'pipeline/scripts/generate-context.sh', readTemplate('scripts/generate-context.sh'), written);

  // Make scripts executable
  for (const script of ['pipeline/scripts/dev-loop.sh', 'pipeline/scripts/setup.sh', 'pipeline/scripts/generate-context.sh']) {
...
function appendGitignore(cwd) {
  ...
  const addition = 'pipeline/logs/*.log';
