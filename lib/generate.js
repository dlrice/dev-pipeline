...
Do not scan src/ to understand the project. CONTEXT.md already summarises
every file. Only read specific source files when you need to modify them.
...
Before starting any plan or implementation, assess the scope. If the task
exceeds single-session capacity (more than 8 new files, more than 50 tests,
more than 500 lines of new code, or more than 2 independent concerns),
output \`COMPLEXITY_LIMIT: This feature should be split.\` and generate a
...
Do not scan src/ to understand the project. CONTEXT.md already summarises
every file. Only read specific source files when your task requires it.
...
## Complexity Detection
Before starting any task, assess the scope of the spec. If the feature
describes more than 3-4 screens, introduces more than 5 new components,
requires more than 3 API endpoints, or contains multiple independently-
shippable features, output \`COMPLEXITY_LIMIT: This feature should be split.\`
and generate a decomposition to \`specs/{feature-name}-decomposition.md\`
