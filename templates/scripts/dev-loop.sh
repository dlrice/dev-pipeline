...
mkdir -p pipeline/logs pipeline/reviews/spec pipeline/reviews/plan \
         pipeline/reviews/implementation pipeline/tests/gemini pipeline/tests/qwen \
         pipeline/plans pipeline/signals tests
...
if should_run 0; then
    ...
    while true; do
        ...
        GEMINI_OUT="pipeline/reviews/spec/${FEATURE}-gemini-r${REVIEW_ROUND}.md"
        QWEN_OUT="pipeline/reviews/spec/${FEATURE}-qwen-r${REVIEW_ROUND}.md"
...
" --dangerously-skip-permissions 2>&1 | tee /tmp/claude-spec-merge-r${REVIEW_ROUND}.out || MERGE_EXIT=$?
    ...
    git add "$SPEC_FILE" pipeline/reviews/spec/
...
" --dangerously-skip-permissions 2>&1 | tee /tmp/claude-sessionA1.out || CLAUDE_A1_EXIT=$?
    ...
    while true; do
        ...
        PLAN_GEMINI="pipeline/reviews/plan/${FEATURE}-gemini-r${REVIEW_ROUND}.md"
        PLAN_QWEN="pipeline/reviews/plan/${FEATURE}-qwen-r${REVIEW_ROUND}.md"
...
"
    ...
    git add "$PLAN_FILE" "$SPEC_FILE" pipeline/reviews/plan/
...
" --continue "$SESSION_B_ID" --dangerously-skip-permissions 2>&1 | tee /tmp/claude-sessionB1-heal.out
    ...
    IMPL_GEMINI="pipeline/reviews/implementation/${FEATURE}-gemini.md"
    IMPL_QWEN="pipeline/reviews/implementation/${FEATURE}-qwen.md"
...
echo "    Plan:           $PLAN_FILE"
echo "    Tests:          tests/"
echo "    Spec reviews:   pipeline/reviews/spec/"
echo "    Plan reviews:   pipeline/reviews/plan/"
echo "    Impl reviews:   pipeline/reviews/implementation/"
echo "    Log:            $LOG_FILE"
echo ""
