 ...
 * 4. Invokes the dev-loop.sh script bundled with the package
 */
async function run(feature, extraArgs) {
  ...
  // Locate the dev-loop.sh bundled with this package
  const scriptDir = path.join(__dirname, '..', 'templates', 'scripts');
  const devLoop = path.join(scriptDir, 'dev-loop.sh');

  if (!fs.existsSync(devLoop)) {
    console.error('  Error: dev-loop.sh not found in package. Reinstall aidev.');
    process.exit(1);
  }
  ...
  // Pass package script directory as env var so dev-loop.sh can find generate-context.sh
  const env = {
    ...process.env,
  ...
  const child = spawn('bash', cmdArgs, {
    cwd,
    env,
    stdio: 'inherit',
  });
