...
async function doctor() {
  ...
  const configExists = fs.existsSync('aidev.config.json');
  if (configExists) {
    console.log(`    ${GREEN}✓${NC} aidev.config.json`);
  } else {
    console.log(`    ${YELLOW}⚠${NC} aidev.config.json not found (run ${BOLD}aidev init${NC} to create)`);
  }

  const specsExists = fs.existsSync('aidev/specs/_template.md');
  if (specsExists) {
    console.log(`    ${GREEN}✓${NC} aidev/specs/_template.md`);
  } else {
    console.log(`    ${YELLOW}⚠${NC} aidev/specs/ not found (run ${BOLD}aidev init${NC} to create)`);
  }
  ...
  if (allGood) {
    ...
    console.log(`  For tool installation: ${BOLD}aidev setup${NC}`);
  }
  console.log('');
}
